...
function FreshForm({ onClose, item }) {
  ...
  // wait until mandis data loads
  if (!form.district || mandisData.length === 0) {
    return;
  }
  ...
  return (
    <div className="fresh-item-form">
      ...
      {/* PRICE PER KG */}
      <input name="price" type="number" value={form.price} placeholder="Price per kg (₹)" onChange={handleChange}/>
      ...
      <div className="form-actions">
        ...
        <button type="button" className="add-btn" onClick={handleSubmit} disabled = {loadingSubmit}>{loadingSubmit? (item ? "Updating..." : "Adding..."): (item ? "Update Item" : "➕ Add Item")}</button>
      </div>
    </div>
  );
}
